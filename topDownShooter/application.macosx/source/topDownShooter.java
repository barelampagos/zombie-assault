...
public void drawText() {
  ...
  //TOP TEXT
  text("Health", width*.25f, 30);
  stroke(255, 0, 0);
  fill(255, 0, 0, 90);
  for (int i = 0; i < playerCharacter.health; i++) {
    ellipse(width*.22f + (40 * i), 50, 30, 30);
  }
  fill(255);
  text("Score", width/2, 30); 
  text(score, width/2, 70);
  text("Remaining  Enemies:", width*.75f, 30); 
  text(remainingEnemies, width*.75f, 70);
  ...
  if (waveComplete) {
    fill(0xff00F00D); // Filled with F00D
    textSize(60);
...
public void spawnEnemy() {
  ...
  int c = color(0, random(200, 255), 0);
  if (r == 0) {
...
public boolean checkCollision(float x, float y, float x2, float y2, float r) {
  if (dist(x, y, x2, y2) < r) {
    return true;
  } else {
    return false;
...
class Player {
  ...
  int movementSpeed;
  float angle;
  int fill, stroke, projectileFill;
...
class Projectile {
  ...
  Projectile(Player p, int projectileFill, PVector location, PVector mouseLoc) {
    this.p = p;
...
class Enemy {
  ...
  Enemy(PVector location, Player p, int fill) {
    this.location = location;
    this.p = p;
    size = 50;
...
class PowerUp {
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "topDownShooter" };
    if (passedArgs != null) {
